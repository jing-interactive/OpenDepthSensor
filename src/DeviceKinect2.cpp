...
#define SCOPED_COM_OBJECT(objName) auto scoped##__LINE__## = msw::makeComUnique(objName);
    ...
    struct DeviceKinect2 : public Device
    {
        ...
        static uint32_t getDeviceCount() { return 1; }
        ...
        ivec2 getDepthSize() const { return {depthDesc.width, depthDesc.height}; }

        ivec2 getColorSize() const { return {colorDesc.width, colorDesc.height}; }
        ...
        DeviceKinect2(Option option)
        {
            ...
            if (FAILED(hr))
                CI_LOG_E("KCBGetICoordinateMapper() fails.");
            coordMapper->GetDepthCameraIntrinsics(&cameraIntrinsics);
            if (FAILED(hr))
                CI_LOG_E("GetDepthCameraIntrinsics() fails.");
            ...
            if (option.enableDepth)
            {
                ...
                if (SUCCEEDED(hr))
                {
                    ...
                    depthChannel = Channel16u(depthDesc.width, depthDesc.height,
                                              depthDesc.bytesPerPixel * depthDesc.width, 1,
                                              depthFrame->Buffer);
                }
                if (FAILED(hr))
                    CI_LOG_E("KCBCreateDepthFrame() fails.");
            }
            ...
            if (option.enableInfrared)
            {
                ...
                if (SUCCEEDED(hr))
                {
                    ...
                    infraredChannel = Channel16u(infraredDesc.width, infraredDesc.height,
                                                 infraredDesc.bytesPerPixel * infraredDesc.width, 1,
                                                 infraredFrame->Buffer);
                }
                if (FAILED(hr))
                    CI_LOG_E("KCBCreateInfraredFrame() fails.");
            }
            ...
            if (option.enableColor)
            {
                ...
                if (SUCCEEDED(hr))
                {
                    ...
                    colorSurface = Surface8u(colorFrame->Buffer, colorDesc.width, colorDesc.height,
                                             colorDesc.bytesPerPixel * colorDesc.width,
                                             SurfaceChannelOrder::BGRX);
                }
                if (FAILED(hr))
                    CI_LOG_E("KCBCreateColorFrame() fails.");
            }
            ...
            if (option.enablePointCloud && option.enableColor)
            {
                ...
                depthToColorTable =
                    Surface32f(depthDesc.width, depthDesc.height, false, SurfaceChannelOrder::RGB);
            }
            ...
            if (option.enableFace)
            {
                for (int i = 0; i < BODY_COUNT; i++)
                {
                    ...
                    hr = hdFace.frameSource->OpenModelBuilder(
                    ...
                    if (FAILED(hr) || hdFace.faceAlignment == nullptr)
                        break;

                    hr = ::CreateFaceModel(1.0f, FaceShapeDeformations::FaceShapeDeformations_Count,
                                           hdFace.deformations, &hdFace.faceModel);
                    if (FAILED(hr) || hdFace.faceModel == nullptr)
                        break;
                }
                if (FAILED(hr))
                    CI_LOG_E("Failed to setup HD Face.");
        ...
        const vec3 toCi(const CameraSpacePoint& pos) { return vec3(pos.X, pos.Y, pos.Z); }

        const vec2 toCi(const ColorSpacePoint& pos) { return vec2(pos.X, pos.Y); }

        const vec2 toCi(const DepthSpacePoint& pos) { return vec2(pos.X, pos.Y); }
            ...
            if (option.enableDepth && KCBIsFrameReady(sensor, FrameSourceTypes_Depth))
            {
                ...
                if (SUCCEEDED(KCBGetDepthFrame(sensor, depthFrame)))
                {
                    ...
                    if (option.enablePointCloud && option.enableColor)
                    {
                        ...
                        HRESULT hr = KCBMapDepthFrameToColorSpace(
                            sensor, depthPointCount, depthChannel.getData(), depthPointCount,
                ...
                if (depthToCameraTable.getWidth() == 0)
                {
                    ...
                    if (SUCCEEDED(hr))
                    {
                        depthToCameraTable = Surface32f(depthDesc.width, depthDesc.height, false,
            ...
            if (option.enableBody && KCBIsFrameReady(sensor, FrameSourceTypes_Body))
            {
                ...
                if (SUCCEEDED(KCBGetBodyData(sensor, BODY_COUNT, srcBodies, &timeStamp)))
                {
                    ...
                    for (int i = 0; i < BODY_COUNT; i++)
                    {
                        ...
                        Body body;
                        if (!updateBody(srcBodies[i], body))
                            continue;
                        ...
                        Face face;
                        if (!updateFace(body, hdFaces[i], face))
                            continue;
        ...
        bool updateFace(const Body& body, HDFaceInternal& hdFace, Face& face)
        {
            ...
            hr = hdFace.faceModel->CalculateVerticesForAlignment(
                hdFace.faceAlignment, face.vertices.size(),
                (CameraSpacePoint*)face.vertices.data());
            if (FAILED(hr))
                return false;
        ...
        void buildFaceModel(HDFaceInternal& hdFace)
        {
            ...
            else
            {
                ...
                // Collection Status
                if (collection >= FaceModelBuilderCollectionStatus::
                                      FaceModelBuilderCollectionStatus_TiltedUpViewsNeeded)
                {
                    std::cout << "Need : Tilted Up Views" << std::endl;
                }
                else if (collection >= FaceModelBuilderCollectionStatus::
        ...
        bool updateBody(IBody* src, Body& dst)
        {
            ...
            for (auto& mapping : mappingPairs)
            {
                ...
                coordMapper->MapCameraPointToDepthSpace(srcJoints[mapping.second].Position,
                                                        &depthPoint);
    ...
    uint32_t getKinect2Count() { return DeviceKinect2::getDeviceCount(); }

    DeviceRef createKinect2(Option option) { return DeviceRef(new DeviceKinect2(option)); }
} // namespace ds
