...
int KinectList::Add(_In_ IKCBSensor *pSensor)
{
    int kcbHandle = m_iNextHandleID++;
...
HRESULT KinectList::Add(int handle, _In_ IKCBSensor *pSensor)
{
    // ensure this was not already added
...
HRESULT KinectList::Remove(int handle, _COM_Outptr_ IKCBSensor** ppSensor)
{
    auto iter = m_pKCBMap.find( handle );
    if( iter != m_pKCBMap.end() )
    {
...
// caller to release the object
HRESULT KinectList::Find(int handle, _COM_Outptr_ IKCBSensor** ppSensor)
{
    auto iter = m_pKCBMap.find( handle );
    if( iter != m_pKCBMap.end() )
    {
